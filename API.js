...
async function sendPaymentRequestEmail(email) {
    try {
        const upiId = 'rishitgoklani@oksbi';
        const mailOptions = {
            from: 'your-email@gmail.com',
            to: email,
            subject: 'Payment Required for Priority Photo Processing',
...
// Existing FIFO top-job-download (first in, first out)
router.get('/top-job-download', async (req, res) => {
    try {
        const paidJobs = await Job.find({ 
            status: 'pending',
        ...
        const { email, photoPath } = topJob;
        console.log("Top job for download (FIFO):", { email, photoPath, paid: topJob.paid });
        res.download(photoPath, path.basename(photoPath));
    } catch (error) {
        console.error("Error retrieving top job (FIFO):", error.message);
        res.status(500).json({ message: 'Error retrieving top job', error: error.message });
    }
});
...
router.post('/pop-top-job', upload.single('photo'), async (req, res) => {
    try {
        ...
        await sendPhotoEmail(topEmail, photoPath);
        await Job.deleteOne({ _id: topJob._id });
        console.log("Top job popped (FIFO):", { email: topEmail });
        const nextJob = await Job.findOne({ status: 'pending' }).sort({ priority: 1 });
        if (nextJob) {
            console.log("Next job scheduled (FIFO):", {
                email: nextJob.email,
                photoPath: nextJob.photoPath
            });
        } else {
            console.log("No more pending jobs in queue (FIFO)");
        }
        res.status(200).json({
            message: 'Photo sent to top priority email and job removed successfully',
            email: topEmail
        ...
        console.error("Error popping top job (FIFO):", error.message);
        res.status(500).json({
            message: 'Error popping top job or sending email',
            error: error.message
        });
...
// New LIFO top-job-download (last in, first out)
router.get('/top-job-download-lifo', async (req, res) => {
    try {
        const paidJobs = await Job.find({ 
            status: 'pending',
            paid: true 
        }).sort({ priority: -1 }); // Sort descending for LIFO
        let topJob = paidJobs[0];
        if (!topJob) {
            const unpaidJobs = await Job.find({ 
                status: 'pending',
        ...
        if (!topJob) {
            console.log("No pending jobs found in either category (LIFO)");
            return res.status(404).json({ message: 'No pending jobs in the queue' });
        }
        const { email, photoPath } = topJob;
        console.log("Top job for download (LIFO):", { email, photoPath, paid: topJob.paid });
        res.download(photoPath, path.basename(photoPath));
    } catch (error) {
        console.error("Error retrieving top job (LIFO):", error.message);
        res.status(500).json({ message: 'Error retrieving top job', error: error.message });
    }
});

// New LIFO pop-top-job
router.post('/pop-top-job-lifo', upload.single('photo'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: 'No photo uploaded' });
        }
        ...
        await sendPhotoEmail(topEmail, photoPath);
        await Job.deleteOne({ _id: topJob._id });
        console.log("Top job popped (LIFO):", { email: topEmail });
        const nextJob = await Job.findOne({ status: 'pending' }).sort({ priority: -1 });
        if (nextJob) {
            console.log("Next job scheduled (LIFO):", {
                email: nextJob.email,
                photoPath: nextJob.photoPath
            });
        } else {
            console.log("No more pending jobs in queue (LIFO)");
        }
        res.status(200).json({
            message: 'Photo sent to top priority email and job removed successfully',
            email: topEmail
        ...
        console.error("Error popping top job (LIFO):", error.message);
        res.status(500).json({
            message: 'Error popping top job or sending email',
            error: error.message
        });
